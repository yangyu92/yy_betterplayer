...
class _ReusableVideoListWidgetState extends State<ReusableVideoListWidget> {
  ...
  void _freeController() {
    ...
    if (controller != null && _initialized) {
      ...
      controller?.dispose();
      controller = null;
      print("关闭之前的播放，将controller设置为空");
      if (!betterPlayerControllerStreamController.isClosed) {
        betterPlayerControllerStreamController.add(null);
        print("移除了StreamController");
      }
      _initialized = false;
    }
  }
        ...
        children: [
          ...
          VisibilityDetector(
            ...
            onVisibilityChanged: (info) {
              ...
              print('info: ${info.visibleBounds}');
              print("videoTitle: ${videoListData?.videoTitle}");
              if (!widget.canBuildVideo!()) {
                _timer?.cancel();
                _timer = null;
                _timer = Timer(Duration(milliseconds: 500), () {
                  print("canBuildVideo: true");
